...
#ifdef CCORE_CLOCK_TYPE_REALTIME
...
EventTypeWrapper EventPosix::Wait(unsigned long timeout) {
  ...
  if (kDown == state_) {
    if (CCORE_EVENT_INFINITE != timeout) {
      timespec end_at;
#ifndef CCORE_MAC
#ifdef CCORE_CLOCK_TYPE_REALTIME
...
bool EventPosix::StartTimer(bool periodic, unsigned long time) {
  ...
  const char* thread_name = "event_timer_thread";
...
bool EventPosix::Process() {
  ...
  end_at.tv_sec  = long(created_at_.tv_sec + time / 1000);
  end_at.tv_nsec = long(created_at_.tv_nsec + (time - (time / 1000) * 1000) * E6);
