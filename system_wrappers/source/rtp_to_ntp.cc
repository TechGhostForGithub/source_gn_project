...
bool RtpToNtpMs(int64_t rtp_timestamp,
  ...
  if (!CompensateForWrapAround((uint32_t)rtcp_timestamp_new,
  ...
  if (!CompensateForWrapAround((uint32_t)rtp_timestamp, (uint32_t)rtcp_timestamp_old,
  ...
  *rtp_timestamp_in_ms = (int64_t)rtp_timestamp_ntp_ms;
  return true;
}
